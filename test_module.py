...
class UnitTests(unittest.TestCase):
    def test_port_scanner_ip(self):
        actual = port_scanner.get_open_ports("209.216.230.240", [440, 445], False)
        expected = [443]
        self.assertEqual(actual, expected, 'Expected scanning ports of IP address to return [443].')

    def test_port_scanner_url(self):
        actual = port_scanner.get_open_ports("www.stackoverflow.com", [79, 82], False)
        expected = [80]
    ...
    def test_port_scanner_url_multiple_ports(self):
        actual = port_scanner.get_open_ports("scanme.nmap.org", [20, 80], False)
        expected = [22, 80]
    ...
    def test_port_scanner_verbose_ip_no_hostname_returned_single_port(self):
        result = port_scanner.get_open_ports("104.26.10.78", [440, 450], True)
        actual = result
